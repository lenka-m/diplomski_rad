...
function AllActivitiesAdmin({ loggedUser }) {
  ...
  function handleFilter(value) {
    if (
      ...
      (filterValue.isSet === true && filterValue.activityStatus !== value)
    ) {
      setFilteredActivities(
        activities.filter((activity) => activity.status === value)
      );
  ...
  function handleAccept(activity) {
    try {
      adminPatchActivity({
        ...
        numOfPoints: activity.numOfPoints,
      }).then(() => {
        fetchAllActivities().then(() => {
          setConfirmActivitySuccess({
            isSuccess: true,
            message: "Uspesno prihvacena aktivnost",
          });
          setTimeout(() => {
  ...
  function handleUpdatePoints() {
    ...
    try {
      console.log("pozivamo handle update");
  ...
  function handleDeleteActivity(activity) {
    try {
      deleteActivity(activity.id).then(fetchAllActivities());
      setConfirmActivitySuccess({
        isSuccess: true,
        message: "Uspesno odbijena aktivnost",
      });
      setTimeout(() => {
        setConfirmActivitySuccess({ isSuccess: null, message: "" });
      }, 2000);
  ...
  return (
    <div className="HomepageContainer">
      {activities.length === 0 ? (
        <h1 className="tableTitle">Nema Aktivnosti :D</h1>
      ) : (
        <div style={{ width: "100%" }}>
          <div className="titleContainer">
            ...
            <div className="rightContainerRow">
              <button
                className={
                  filterValue.activityStatus === "pending"
                    ? "clicked"
                    : "greenBtn"
                }
                value="pending"
              ...
              <button
                className={
                  filterValue.activityStatus === "created"
                    ? "clicked"
                    : "greenBtn"
                }
                value="created"
      ...
      {confirmActivitySuccess.isSuccess === true && (
        <Alert severity="success">{confirmActivitySuccess.message}</Alert>
      )}
      {confirmActivitySuccess.isSuccess === false && (
        <Alert severity="error">{confirmActivitySuccess.message}</Alert>
      )}
    </div>
  );
}
